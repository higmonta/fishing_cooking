class CookingMemoriesController < ApplicationController
  ...
  def index
    ...
    @current_user_cooking_memories = @cooking_memories.where(user_id: current_user.id).order(cooking_date: :DESC, created_at: :DESC)
  end

  def edit
    @cooking_memory = CookingMemory.find(params[:id])
    return if @cooking_memory.user == current_user

...
RSpec.describe 'CookingMemories', type: :system do
  ...
  describe 'ログイン後の料理履歴の削除機能の検証' do
    ...
    context '画像を添付していない時' do
      it '料理履歴の削除に成功する' do
        not_attached_image_cooking_memory = create(:cooking_memory, :not_attached_image, user: user)
        visit cooking_memories_path
        click_on '削除'
        expect(page.accept_confirm).to have_content 'こちらの料理履歴を削除してもよろしいですか？'
        expect(page).to have_content '削除に成功しました'
  ...
  describe 'ログイン後の料理履歴の検索' do
    ...
    context '魚の種類を検索条件に入れて検索した時' do
      it '検索条件に入れた魚の名前が含まれる料理が検索できる' do
        cooking_memory_list = create_list(:cooking_memory, 3, :attached_image, user: user)
        cooking_memory_another_fish_name_list = create_list(:cooking_memory, 3, :attached_image_another_fish_name, user: user)
        fill_in 'q_fish_name_cont', with: 'test_fish_name'
        find('.cooking-memories-search-button').click
        expect(page).to have_content cooking_memory_list[0].fish_name
        expect(page).to have_content cooking_memory_list[1].fish_name
        expect(page).to have_content cooking_memory_list[2].fish_name
        expect(page).to_not have_content cooking_memory_another_fish_name_list[0].fish_name
        expect(page).to_not have_content cooking_memory_another_fish_name_list[1].fish_name
        expect(page).to_not have_content cooking_memory_another_fish_name_list[2].fish_name
        expect(current_path).to eq cooking_memories_path
      end
    end
    ...
    context 'キーワードの欄に検索条件を入れて検索した時' do
      ...
      it 'キーワードに含まれている料理名の料理が検索できる' do
        cooking_memory_list = create_list(:cooking_memory, 3, :attached_image, user: user)
        cooking_memory_another_cooking_name_list = create_list(:cooking_memory, 3, :attached_image_another_cooking_name, user: user)
        fill_in 'q_fish_name_or_cooking_name_or_memo_cont', with: 'test_cooking_name'
        find('.cooking-memories-search-button').click
        expect(page).to have_content cooking_memory_list[0].cooking_name
        expect(page).to have_content cooking_memory_list[1].cooking_name
        expect(page).to have_content cooking_memory_list[2].cooking_name
        expect(page).to_not have_content cooking_memory_another_cooking_name_list[0].cooking_name
        expect(page).to_not have_content cooking_memory_another_cooking_name_list[1].cooking_name
        expect(page).to_not have_content cooking_memory_another_cooking_name_list[2].cooking_name
        expect(current_path).to eq cooking_memories_path
      end

      it 'キーワードに含まれているメモの料理が検索できる' do
        cooking_memory_list = create_list(:cooking_memory, 3, :attached_image, user: user)
        cooking_memory_another_fish_name_list = create_list(:cooking_memory, 3, :attached_image_another_memo, user: user)
        fill_in 'q_fish_name_or_cooking_name_or_memo_cont', with: 'test_memo'
        find('.cooking-memories-search-button').click
        expect(page).to have_content cooking_memory_list[0].memo
        expect(page).to have_content cooking_memory_list[1].memo
        expect(page).to have_content cooking_memory_list[2].cooking_name
        expect(page).to_not have_content cooking_memory_another_fish_name_list[0].memo
        expect(page).to_not have_content cooking_memory_another_fish_name_list[1].memo
        expect(page).to_not have_content cooking_memory_another_fish_name_list[2].memo
        expect(current_path).to eq cooking_memories_path
      end
    end
